...
#ifndef ForFlipBoard_Flip_h
#define ForFlipBoard_Flip_h
...
static inline CATransform3D JHFlipCATransform3DMakePerspective(CGPoint center, float disZ)
{
...
static inline CATransform3D JHFlipCATransform3DPerspect(CATransform3D t, CGPoint center, float disZ)
{
    return CATransform3DConcat(t, JHFlipCATransform3DMakePerspective(center, disZ));
}
static inline CATransform3D JHFlipCATransform3DPerspectSimple(CATransform3D t){
    return JHFlipCATransform3DPerspect(t, CGPointMake(0, 0), 3000.0f);
}
static inline CATransform3D JHFlipCATransform3DPerspectSimpleWithRotate(CGFloat degree){
    return JHFlipCATransform3DPerspectSimple(CATransform3DMakeRotation((M_PI*degree/180.0f), 1.0, 0.0, 0.0));
}


///为UIView创建一个截图
static inline UIImage* JHFlip_make_image_for_view(UIView* view){
...
///为UIView创建截图并且分割
static inline NSArray* JHFlip_make_hsplit_images_for_view(UIView* view){
    UIImage* image=JHFlip_make_image_for_view(view);
    return JHFlip_make_hsplit_images_for_image(image);
}
/**
 *  线性背景图
 *
 ...
 *  @return
 */
static inline UIImage *JHFlip_line_gradient_image(CGRect rect){
    if(UIGraphicsBeginImageContextWithOptions != NULL){
...
static inline UIImage *JHFlip_line_radial_image(CGRect rect){
    // Drawing code
    if(UIGraphicsBeginImageContextWithOptions != NULL){
        UIGraphicsBeginImageContextWithOptions(rect.size, NO, 0.0);
    } else {
